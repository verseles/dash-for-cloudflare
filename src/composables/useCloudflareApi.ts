...
export function useCloudflareApi() {
  ...
  const cfFetch = async <T>(endpoint: string, options: RequestInit = {}): Promise<T> => {
    ...
    if (!response.ok) {
      ...
      const errorMessage =
        errorData.errors?.[0]?.message || `HTTP error! status: ${response.status}`;
  ...
  const updateDnsRecord = async (zoneId: string, record: DnsRecord): Promise<DnsRecord> => {
  ...
  const deleteDnsRecord = async (zoneId: string, recordId: string): Promise<{ id: string }> => {
