<template>
  <q-page>
    ...
    <div class="dns-content" :key="selectedZoneId || 'no-zone'" style="padding-bottom: 64px">
      ...
      <div
        v-else-if="filteredRecords.length === 0 && selectedZoneId && !isLoadingRecords"
        class="q-pa-md"
      >
      ...
      <q-list v-else-if="filteredRecords.length > 0 && !isLoadingRecords">
        <DnsRecordItem
          ...
          :key="`${selectedZoneId}-${record.id}`"
          :record="record"
          :is-saving="savingRecordIds.has(record.id)"
          :is-new-record="newRecordIds.has(record.id)"
          :is-deleting="deletingRecordIds.has(record.id)"
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 80]">
      <q-btn
        ...
        color="primary"
        :outline="true"
        :disable="!selectedZoneId || isLoadingZones"
        @click="handleAdd"
      >
...
const { records, isLoadingRecords, saveRecord, deleteRecord } = useDnsManagement();
